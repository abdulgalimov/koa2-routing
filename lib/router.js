...
function Router(app) {
    if (this instanceof Router) {
        ...
        app.route = function(path,domain) {
            // self.routes.map(function(_route){
            //     // todo
            //     // if(_route._domain !== domain || _route.path !== path){
            //     //     //
            //     // }else{
            //     //     //
            //     // }
            //     //同域名下不能有相同的路径  * 下不能有相同的  *需要最后追加   domain 和path 都不能对 正则进行判断，需要自行校验
            //     assert( _route._domain !== domain || _route.path !== path ,`You already defined path"${path}" for "${domain}"`)
            // })
